...
class CertificateApp {
  ...
  loadImageFile(file) {
    return new Promise((resolve, reject) => {
      ...
      this.bgImg.onload = () => {
        ...
        this.applyAutoColorsFromBackground();
        resolve();
      };
  ...
  applyAutoColorsFromBackground() {
    // استخدام ColorExtractor (لو متوفر) لاستخراج ألوان بارزة من الخلفية
    if (!window.ColorExtractor || !this.bgImg || !this.bgImg.naturalWidth) return;

    try {
      const extractor = new window.ColorExtractor(this.bgImg);
      const colors = extractor.extractPalette();
      if (colors && colors.length > 0) {
        // أول لون لاسم المستلم
        this.nameColorInput.value = colors[0];
        // ثاني لون – إن وجد – لنص الشهادة
        if (colors[1]) {
          this.bodyColorInput.value = colors[1];
        }
        this.updateTexts();
      }
    } catch (err) {
